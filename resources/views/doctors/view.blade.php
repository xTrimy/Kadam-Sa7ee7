<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="flex justify-between p-6 bg-white border-b border-gray-200">
                    <h1 class="text-xl">
                        ...
                        {{ __('Total Count') }} : {{ $doctors->count() }}
                        </p>
                    </h1>
                    @if (auth()->user()->hasPermissionTo('Add doctor'))
    ...
    @if(method_exists($doctors, 'links'))
        {{ $doctors->appends(Request::except(['page','_token']))->links() }}
    @endif
    </div>
</div>
